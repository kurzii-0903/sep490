...
export const AuthProvider = ({ children }) => {
    ...
    const login = async (email, password) => {
        try {
            const response = await axios.post(`/api/Authentication/authenticate/loginby-email-password`, {
                email,
                password,
            });
    ...
    const LoginGoogle=({ setUser ,setError})=> {
        ...
        const onSuccess = async (googleResponse) => {
            try {
                ...
                const res = await axios.post(`/api/Authentication/authenticate/loginby-google`,
