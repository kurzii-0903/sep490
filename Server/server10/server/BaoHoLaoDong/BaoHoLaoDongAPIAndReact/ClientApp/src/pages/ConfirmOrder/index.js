...
export function ConfirmOrder() {
  ...
  useEffect(() => {
    if (
      ...
      showNotification(
        "Giỏ hàng của bạn trống hoặc dữ liệu không hợp lệ. Vui lòng kiểm tra lại!",
        "error"
      );
      setTimeout(() => {
        navigate("/cart");
      ...
      console.log("orderData:", orderData); // Log để kiểm tra
    }
  }, [orderData, navigate]);
  ...
  const calculateOrder = async () => {
    if (
      ...
      return; // Không tính toán nếu đơn hàng rỗng
    }
      ...
      const newOrder = {
        ...
        orderDetails: orderData.orderDetails.map(
          ({ productId, quantity, variant }) => ({
            ...
            size: variant.size || null, // Thêm size
            color: variant.color || null, // Thêm color
          })
        ),
      };
      const response = await axios.post(`/api/Order/calculate-order`, newOrder);
  ...
  const totalAmount =
    orderData && orderData.orderDetails && orderData.orderDetails.length > 0
                      ...
                      {orderData.orderDetails.map(
                        (
                          ...
                          >
                            ...
                            <td className="border px-3 py-2 text-center">
                              {calculatedOrder.orderDetails?.[index]
                                ?.productTax || 0}
                              %
                            </td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    className={`mt-1 p-2.5 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 ${
              ...
              <div className="text-right">
                <button
                  className={`px-8 py-3 rounded-lg text-white font-medium text-lg ${
                    isSubmitting || !orderData.orderDetails
                      ? "bg-gray-400 cursor-not-allowed"
                      : "bg-red-500 hover:bg-red-600 active:bg-red-500"
                  }`}
                  onClick={handleOrder}
                  disabled={isSubmitting || !orderData.orderDetails}
                >
                  {isSubmitting
                    ? "Đang xử lý..."
                    : !orderData.orderDetails
