...
function Header({ cartItems, removeFromCart, updateCartItemQuantity, showToast }) {
    ...
    const getAvatar = () => {
        console.log("User data:", { imageUrl: user?.imageUrl, gender: user?.gender }); // Debug chi tiết
        if (user?.imageUrl) return user.imageUrl; // Ưu tiên imageUrl (Google hoặc cơ sở dữ liệu)

        const gender = user?.gender;
        if (typeof gender === "string") {
            const genderLower = gender.toLowerCase();
            if (["male", "nam", "man", "male"].includes(genderLower)) return maleAvatar;
            if (["female", "nữ", "woman", "female"].includes(genderLower)) return femaleAvatar;
        } else if (typeof gender === "boolean") {
            return gender ? maleAvatar : femaleAvatar; // true: Male, false: Female
        } else if (typeof gender === "number") {
            return gender === 1 ? maleAvatar : gender === 0 ? femaleAvatar : maleAvatar; // Xử lý số (nếu có)
        }

        return maleAvatar; // Mặc định là nam
    };
