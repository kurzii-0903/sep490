...
export default function CreateOrder() {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try{
            const response = await axios.post(`/api/Order/create-order-v2`, newOrder);
    ...
    const addToOrder = () => {
        ...
        if (product.productVariants && product.productVariants.length !== 0) {
            ...
            newItem = {
                ...
                quantity,
                totalPrice: (selectedVariant.price*(1+(product.discount/100)) || 0) * quantity,
                name: product.name,
                size: selectedVariant.size || null,
                color: selectedVariant.color || null, // Fixed: was using size instead of color
            ...
            newItem = {
                productId: product.id,
                variantId: null, // Fixed: was using selectedVariant which might be null
                productPrice: product.price*(1+(product.discount/100)) || 0,
                quantity,
                totalPrice: (product.price*(1+(product.discount/100)) || 0) * quantity,
                name: product.name,
                size: null,
                color: null,
                image: product.image || noImage,
        ...
        setOrder((prevOrder) => {
            ...
            const existingIndex = newOrderDetails.findIndex(
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8">
                ...
                <div className="grid grid-cols-4 gap-4">
                    {product.productImages &&
                        product.productImages.map((img, index) => (
                            <div
                                key={index}
                                className="aspect-square rounded-md overflow-hidden bg-gray-100 cursor-pointer hover:ring-2 ring-blue-500"
                            >
                                <img
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center space-x-4">
                    <span className="text-3xl font-bold text-red-600">{safeFormatPrice(product.price*(1+(product.discount/100)))}</span>
                    <span className="text-lg text-gray-400 line-through">{safeFormatPrice(product.price)}</span>
                    {product.discount > 0 && (
              ...
              -{product.discount}%
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <span className="text-sm text-gray-500">Còn {product.quantity || 0} sản phẩm</span>
                </div>

                {/* Nút thêm vào đơn hàng */}
                <button onClick={addToOrder} className="w-full bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
                    Thêm vào đơn hàng
                </button>
            </div>
        </div>
