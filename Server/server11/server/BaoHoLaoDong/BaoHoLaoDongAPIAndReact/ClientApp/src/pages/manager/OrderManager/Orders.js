...
const Orders = ({ config }) => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          `/api/Order/get-page-orders?page=${currentPage}&pageSize=10&customerName=${searchName}&startDate=${
            startDateString || ""
          }&endDate=${endDateString || ""}&status=${
            status === "all" ? "" : status
          }`
        );
  ...
  const updateOrderStatus = async (orderId, invoiceNumber, newStatus) => {
    ...
    try {
      ...
      const detailResponse = await axios.get(`/api/Order/get-order/${orderId}`);
