...
const Signin = ({ config }) => {
    ...
    const { login, fetchUser, setUser, LoginGoogle } = useContext(AuthContext); // Thêm LoginGoogle và setUser
    ...
    const handleSignin = async (e) => {
        ...
        setLoading(true);
        try {
            const result = await login(formSignin.email, formSignin.password);
    ...
    return (
        <PageWrapper title="Đăng nhập" config={config}>
