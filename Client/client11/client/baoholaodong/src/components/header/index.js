...
function Header({
    ...
    const fetchPhoneNumber = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/api/BlogPost/get-blog-by-category/lien-he`
        );
        const contactData = response.data;
        const phonePost = contactData.find(
          (post) => post.title.toLowerCase() === "điện thoại"
        );
        if (phonePost) {
  ...
  const isCartPage =
    location.pathname === "/cart" || location.pathname === "/confirm-order";
