...
export function ConfirmOrder({ config }) {
  ...
  const { clearSelectedItems, showToast } = useContext(CartContext); // Use CartContext
  ...
  const calculateOrder = async () => {
    try {
      const newOrder = {
        customerName: customerInfo.customerName || "dinhlinh",
      ...
      const response = await axios.post(
        `${BASE_URL}/api/Order/calculate-order`,
        newOrder
      );
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      showToast(
        "Đơn hàng đã được đặt thành công và giỏ hàng đã được cập nhật!"
      );
            ...
            <div className="p-6 md:p-8 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4 border-t border-gray-200">
                ...
                {customerInfo.paymentMethod === "Online" && invoiceNumber && (
                  <button
                    onClick={() =>
                      (window.location.href = `/checkout?invoiceNumber=${invoiceNumber}`)
                    }
      ...
      >
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="border-t border-gray-200 pt-6 flex flex-col md:flex-row justify-between muscle items-end">
