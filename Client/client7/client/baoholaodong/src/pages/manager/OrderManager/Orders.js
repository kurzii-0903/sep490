...
const Orders = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/api/Order/get-page-orders`,
          {
            params: {
  ...
  // Hàm xử lý khi nhấn nút SquarePen để xem hình ảnh hóa đơn
  const handleViewInvoiceImage = (invoiceNumber) => {
    if (invoiceNumber) {
      ...
      // Nếu không có invoiceNumber, có thể hiển thị thông báo hoặc không làm gì
